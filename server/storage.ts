...
export interface IStorage {
  ...
  getUserByClientRef(clientRef: string): Promise<User | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByClientRef(clientRef: string): Promise<User | undefined> {
    const [user] = await db.select().from(users).where(eq(users.clientRef, clientRef));
    return user;
  }
